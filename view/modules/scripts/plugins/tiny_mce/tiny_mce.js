...
(function() {
	function a(g) {
		...
		this.addRange = function(l) {
			var t, z, y = g.dom.doc, r = y.body, u, n, x, o, s, k, p, q, v, m;
			this.destroy();
			x = l.startContainer;
			o = l.startOffset;
			s = l.endContainer;
			...
			if (x == s && x.nodeType == 1) {
				if (/^(IMG|TABLE)$/.test(x.nodeName) && o != k) {
					t = r.createControlRange();
					t.addElement(x)
				} else {
					t = r.createTextRange();
					if (!x.hasChildNodes() && x.canHaveHTML) {
						x.innerHTML = j
					}
			...
			if (s.nodeType == 3) {
				...
				z.moveToElementText(p);
				p.parentNode.removeChild(p);
				z.move("character", k);
				t.setEndPoint("EndToStart", z)
			} else {
				z.moveToElementText(s);
				z.collapse(!!m);
				t.setEndPoint("EndToEnd", z)
			}
			t.select();
			t.scrollIntoView()
		};
	...
	m
			.create(
					"tinymce.Editor",
					{
						...
						focus : function(s) {
							var v, q = this, u = q.settings.content_editable, r, p;
							if (!s) {
								...
								if (p) {
									r = q.getDoc().body.createControlRange();
									r.addElement(p);
									r.select()
								}
							}
							if (m.activeEditor != q) {
								if ((v = m.activeEditor) != null) {
									v.onDeactivate.dispatch(v, q)
								}
								q.onActivate.dispatch(q, v)
							}
							m._setActive(q)
						},
								...
								T[ad].addToTop(function(ae, af) {
									if (ab()
											&& !a.dom.RangeUtils.compareRanges(
													N.lastRng, p.getRng())) {
										M(c.select("font,span"), function(ai) {
											var ah, aj, ag;
											if (b(ai)) {
												aj = ai.firstChild;
												ac(ai);
